...
public class WeatherApiRestControllerTest {
    ...
    public void testWeatherEndpointWithGETListAndDateFilter() throws Exception {
        LocalDate date = LocalDate.parse("2019-03-12", dateFormatter);
        Map<String, Weather> data = getTestData();
        data.remove("moscow2");
        List<Weather> expectedRecords = new ArrayList<>();
    ...
    public void testWeatherEndpointWithGETListAndCityFilter() throws Exception {
        ...
        expectedRecords = expectedRecords.stream().filter(r -> r.getCity().toLowerCase().equals("moscow")).collect(Collectors.toList());
        ...
        mockMvc.perform(get("/weather?city=berlin"))
                .andDo(print())
                .andExpect(jsonPath("$.*", isA(ArrayList.class)))
                .andExpect(jsonPath("$.*", hasSize(0)))
                .andExpect(status().isOk());
    ...
    @Test
    public void testWeatherEndpointWithPOSTBadRequest() throws Exception {
        Map<String, Object> invalidWeather = new HashMap<>();
        invalidWeather.put("date", "2023-05-15");
        invalidWeather.put("lat", 41.8818f);
        invalidWeather.put("lon", -87.6231f);

        mockMvc.perform(post("/weather")
                .contentType("application/json")
                .content(om.writeValueAsString(invalidWeather)))
                .andDo(print())
                .andExpect(status().isBadRequest());
    }

    private Map<String, Weather> getTestData() {
        Map<String, Weather> data = new LinkedHashMap<>();

        Weather chicago = new Weather(
                LocalDate.parse("2019-06-11", dateFormatter),
                41.8818f,
                -87.6231f,
                "Chicago",
                "Illinois",
        ...
        Weather oakland = new Weather(
                LocalDate.parse("2019-06-12", dateFormatter),
                37.8043f,
                -122.2711f,
                "Oakland",
                "California",
        ...
        Weather london = new Weather(
                LocalDate.parse("2019-03-12", dateFormatter),
                51.5098f,
                -0.1180f,
                "London",
                "N/A",
